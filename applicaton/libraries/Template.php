 ...
 * Template library
 *
 * This is template library for the Codeigniter to implement theme feature in 
 * framework
 *
 * @package CodeIgniter
 * @author  Karan Jilka
 * @since   Version 1.1
 * 
 * https://github.com/karan4230/codeigniter-template-library
 * 
 */
// --------------------------------------------------------------------
class CI_Template
{
    ...
    var $CI;
    var $template;
    var $theme_path = 'application/theme/';
    var $asset_path = 'asset/theme/';
    ...
    public function set_layout($file = "")
    {
        if ($file != "")
        {
            ...
            include_once($this->theme_path . $this->template . 'setting' . EXT);
            
            //sotre layout file
            $this->layout = $file;

            if (isset($layout) && array_key_exists($file, $layout))
            {
                $this->template_ar = $layout[$file];
     ...
     * @return Region content.
     */
    public function get_region($region, $file, $data = array(), $appned = true, $theme = false)
    {
        return $this->_region($region, $file, $data, $appned, $theme, true);
    }
    ...
    public function prepend_metadata($file, $type = "", $theme = true)
    {
        if ($type == "js")
        {
            if ($theme)
            {
                $path = $this->asset_path . $this->template;
     ...
     * @param type $theme 
     */
    private function _region($region, $file, $data = array(), $appned = true, $theme = false, $get_region = false)
    {
        if (array_key_exists($region, $this->template_ar['region']))
        {
            ...
            // return region content but not set in Template.
            if ($get_region)
            {
                $region_html = $this->template_ar['region'][$region];
                $this->template_ar['region'][$region] = '';
                return $region_html;
            }
        }
    }
    ...
    {
        $path = $this->theme_path . $this->template.$this->layout_folder;
